...
import {getSensorHeatmapArccolor, getTempStatuscolor,getHumStatuscolor} from "../../../Utils/SensorHelpers"
...
function HeatmapCanvas (){
    ...
    function circle (x, y, radius, sensor, main, maxCircRadius){
        ...
        this.draw = () =>{
            ...
            contextRef.current.fillStyle = `rgba(${getSensorHeatmapArccolor(this.sensor)}, ${this.opacity})`;
            contextRef.current.arc(x, y, this.radius, 0, 2 * Math.PI);
            contextRef.current.fill();
            if(main){
            contextRef.current.drawImage(this.img, x - (floorDimensions.width-this.imgCrop)/2 , y - (floorDimensions.width-this.imgCrop)/2, floorDimensions.width- this.imgCrop, floorDimensions.width - this.imgCrop);
            }
        ...
        this.update = () =>{
                ...
                this.opacity =  (this.maxRadius - this.radius) / this.maxRadius >= 0.4? this.opacity = 0.4 : this.opacity = (this.maxRadius - this.radius) / this.maxRadius;
                this.draw()
        }
    }
        const initcircles = () =>{
        ...
        if(floorDimensions){
            sensors.forEach(sensor => {
                ...
                const y = floorDimensions.height*sensor.y -floorDimensions.height/2;
                circles.push(new circle(x, y, 0, sensor, true, maxCircleRadius));
                circles.push(new circle(x, y, floorDimensions.width, sensor, false, maxCircleRadius));
                circles.push(new circle(x, y, floorDimensions.width*2, sensor, false, maxCircleRadius));
            })
            }
         }
    ...
    const canvasHover = (evt) => {
        ...
        if(circles)
        circles.forEach(c =>{
            if (distance(c.x, c.y, mouse.x, mouse.y) < c.radius && distance(c.x, c.y, mouse.x, mouse.y) < closestDistance) {
                closestDistance = distance(c.x, c.y, mouse.x, mouse.y);
                hoveredCircle =true;
                setCursorStyle("pointer");
           
                setTooltip(<div className="heatMapToolTip" style={{position: "absolute", top: c.y, left:c.x}}>
                    <p>{c.sensor.name}</p>
                    <p>Temp: <span className={getTempStatuscolor(c.sensor.temperature)}>{c.sensor.temperature}°</span></p>
                    <p>Hum: <span className={getHumStatuscolor(c.sensor.humidity)}>{c.sensor.humidity}%</span></p>
                </div>)
    ...
    if(selectedCircle != tooltipSensor && circles){
       ...
       if(circ[0]){
        setTooltip(<div className="heatMapToolTip" style={{position: "absolute", top: circ[0].y, left:circ[0].x}}>
                    <p>{circ[0].sensor.name}</p>
                    <p>Temp: <span className={getTempStatuscolor(circ[0].sensor.temperature)}>{circ[0].sensor.temperature}°</span></p>
                    <p>Hum: <span className={getHumStatuscolor(circ[0].sensor.humidity)}>{circ[0].sensor.humidity}%</span></p>
                </div>)
